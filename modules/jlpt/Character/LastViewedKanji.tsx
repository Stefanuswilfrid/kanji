...
import { useLastViewedKanji, useLastViewedKanjiActions } from "@/store";
import { useRouter } from "next/router";
import React from "react";
import { toast } from "sonner";

export function LastViewedHanzi() {
  const { handleValueMismatch, hydrateLastViewedKanji } = useLastViewedKanjiActions();
  const LastViewedKanji = useLastViewedKanji();
  ...
  }, [handleValueMismatch, hydrateLastViewedKanji]);
  ...
  React.useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (LastViewedKanji && !isInteracted.current) {
              ...
              return (
                <div className="border border-secondary/10 font-sans mx-auto min-w-[300px] select-none w-fit rounded-full bg-black whitespace-nowrap py-2 pl-6 pr-2 flex items-center gap-3">
                  ...
                  <span className="shrink-0 flex-1">
                    {LastViewedKanji.character} <span className="text-xs">({text.lastVisited})</span>
                  </span>
                  <button
                    ...
                    onClick={() => {
                      router.push(LastViewedKanji.pathname + "&from=last-viewed");
                      toast.dismiss(t);
                    }}
                  >
                    &#x2192;
  ...
  }, [LastViewedKanji, router, text.lastVisited]);
