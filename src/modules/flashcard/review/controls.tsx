...
import { useConfetti } from "@/modules/layout/confetti-provider";
  ...
  return (
    <div className="mt-4 flex flex-wrap gap-2 w-full max-w-80 mx-auto">
          ...
          setReviewResult((prev) => {
            ...
            if (currentIndex === wordsLength - 1) {
              timeout.current = setTimeout(() => {
                const correct = newResult.filter(
                  (result) => result === "correct",
                ).length;
                const percentage = Math.round(
                  (correct / newResult.length) * 100,
                );
      ...
      <button
        ...
        onClick={() =>
          setFlipped(currentIndex === flipped ? null : currentIndex)
        }
      >
