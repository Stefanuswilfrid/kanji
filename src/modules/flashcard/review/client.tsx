...
import { FlashcardReviewContent } from "./content";
import { api } from "@/trpc/client";
import { useFlashcard } from "@/modules/layout/reading-layout";
import { useLocale } from "@/locales/use-locale";

export const FlashcardReviewClient = ({
  category,
  numOfCards,
  chapter,
}: {
        ...
        <main className="max-w-[960px] mx-auto sm:px-8 pb-4">
          ...
          <AnimatePresence mode="wait">
            {isLoading || !data ? (
              <Layout
                key="loading-bar"
                className="absolute w-full max-w-[960px] h-80 grid place-items-center"
              >
              ...
              <Layout key="content">
                <FlashcardReviewContent
                  words={data.filter(
                    (card) => card.entries && card.entries.length > 0,
                  )}
                  onCardClick={() => {}}
                />
              </Layout>
