...
export function getColorClass(
  ...
  const base = map[key] || "";
  return useUnderline
    ? `${base} underline decoration-[1.5px] underline-offset-4`
    : base;
}

export function getFreqRangeLabel(
...
export function getTransliteration(token: any): string {
  ...
  if (token.form?.translit) {
    return Array.isArray(token.form.translit)
      ? token.form.translit.join(" ")
      : token.form.translit;
  }

  // Try form_norm.translit
  if (token.form_norm?.translit) {
    return Array.isArray(token.form_norm.translit)
      ? token.form_norm.translit.join(" ")
      : token.form_norm.translit;
  }

  // Try lemma.translit
  if (token.lemma?.translit) {
    return Array.isArray(token.lemma.translit)
      ? token.lemma.translit.join(" ")
      : token.lemma.translit;
  }
